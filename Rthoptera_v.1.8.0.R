Rthoptera <- function() {
  ...
  page_1 <- function() {
    brochure::page(
        ...
        tagList(
          ...
          fluidPage(
            ...
            fluidRow(
              ...
              column(1, verticalLayout(actionButton("closeApp", "Close App")))
            )
          )
        )
      },
      server = function(input, output, session) {
        ...
        # Observe close button and stop the app
          ...
          fluidPage(
            ...
            fluidRow(
              column(
                2,
                div(class = 'inline',
                    tags$label('New Max. Freq. (kHz):', `for` = 'maxfreq'),
                    selectInput('maxfreq', NULL, choices = c(48, 96, 125))
                )
              ),
              column(
                1, actionButton("downsample", "Downsample")
              )
            ),
            plotlyOutput("audioPlot", height = "440px", width = "1520px")
          )
        )
      },
      server = function(input, output, session) {
        ...
        meanspectrum_plotly <- function(wave,
          ...
          plot_ly(data = mean_spectrum_df, x = ~freq, y = ~mean_amp, type = 'scatter', mode = 'lines', line = list(color = 'white')) %>%
            ...
            layout(
              ...
              margin = list(t = 25, r = 15, b = 50, l = 25),
              shapes = list(
                list(
                  type = "line",
                  x0 = 0,
        ...
        tagList(
          ...
          fluidPage(
            ...
            fluidRow(
              ...
              column(
                1, actionButton("applyFilter", "Apply Filter")
              )
            ),
            mainPanel(
              plotlyOutput("audioPlot", height = "420px", width = "1500px")
            )
          )
        )
      },
        ...
        tagList(
          ...
          fluidPage(
            ...
            mainPanel(plotOutput(
              "wavePlot", brush = brushOpts(id = "waveBrush", direction = "x")
            ))
          )
        )
      },
      server = function(input, output, session) {
        ...
        observeEvent(input$zoomOut, {
          if (!is.null(zoomedRegion())) {
            ...
            newEnd <- min(get(input$waveObject, envir = .GlobalEnv)@samp.rate, zoomedRegion()[2] + zoomRange * 0.1)
            if (newEnd - newStart >= get(input$waveObject, envir = .GlobalEnv)@samp.rate) {
              zoomedRegion(NULL)
            } else {
        ...
        observeEvent(input$saveSelection, {
          req(selectedRegion(), input$waveObject, input$selectionName)
          wave <- get(input$waveObject, envir = .GlobalEnv)
  ...
  page_5 <- function() {
    brochure::page(
      ...
      ui = function(request) {
        tagList(
          h1("Spectral Statistics", style = "font-size: 28px; margin-left: 15px;"),
          nav_links(),
          fluidPage(
            theme = bslib::bs_theme(bootswatch = "darkly"),
            tags$head(tags$style(
      ...
      server = function(input, output, session) {
        ...
        output$dataOutput <- renderDT({
          req(result())
          datatable(result()$data, options = list(
    ...
    brochure::page(
      ...
      ui <- function(request) {
        tagList(
          ...
          fluidRow(
            column(12, plotlyOutput("mean_spectrum", height = "380px", width = "1500px"))
          )
        )
      },
      ...
      server <- function(input, output, session) {
        ...
        extract.meanspec <- function(wave, from = NULL, to = NULL, wl = 2048) {
        ...
        plot.meanspec <- function(wave) {
          ...
          p <- plot_ly(full_spec, x = ~frequency, y = ~amplitude, type = 'scatter', 
            ...
            layout(xaxis = list(title = 'Frequency (kHz)'), 
        ...
        brush_colors <- reactiveVal(c("#E69F00", "#009E73", "#0072B2", "#D55E00",
        ...
        observeEvent(input$add_selection, {
          ...
          plotlyProxy("mean_spectrum", session) %>%
            plotlyProxyInvoke("addTraces", list(
              x = spec$frequency, y = spec$amplitude, type = 'scatter', mode = 'none', fill = 'tozeroy', fillcolor = toRGB(colors[selection_number], alpha = 0.8), name = selection_name,
          ...
          p <- plotly_obj()
          p <- p %>%
            add_trace(x = spec$frequency, y = spec$amplitude, type = 'scatter', mode = 'none', fill = 'tozeroy', fillcolor = toRGB(colors[selection_number], alpha = 0.8), name = selection_name,
  ...
  page_7 <- function(){
    ...
    ui <- function(request){
      ...
      # fluidPage(
        ...
        column(
          ...
          div(
            sidebarPanel(
              ...
              numericInput("peakfinder_threshold", "Peakfinder Threshold", value = 0.005),
        ...
        column(
          ...
          mainPanel(
            plotlyOutput("plot"),
            tableOutput("echeme_data"),
            tableOutput("train_data"),
            tableOutput("peak_data"),
            tableOutput("params"),
            width = 12,
            style = "padding: 10px;"
          )
    ...
    server <- function(input, output, session) {
      ...
      temporal_stats <- function(wave, 
        ...
        time_vector <- seq(0, waveDuration, length.out = length(envelope_vector))  # In seconds
        ...
        peak_data <- peak_data %>%
          mutate(echeme.id = ifelse(peak.index == 1, echeme_id, echeme.id),
                 train.id = ifelse(peak.index == 1, train_id, train.id),
                 peak.id = ifelse(peak.index == 1, 1, peak.id))
        ...
        train_data <- peak_data %>%
          ...
          summarize(
            ...
            # train.period = (lead(train.start) - train.start),
            n.peaks = n(),
            peak.rate = n() / ((max(peak.time) - min(peak.time))/1000)
          ) %>%
          ungroup() %>%
        ...
        # mutate(train.period = ifelse((lead(train.start) - train.start) > (max_train_gap*1000), 
        #                              NA, lead(train.start) - train.start))
        ...
        echeme_data <- train_data %>%
          ...
          summarize(
            echeme.start = min(train.start),
            echeme.end = max(train.end),
            echeme.duration = round(echeme.end - echeme.start,2),
            echeme.period = round((lead(echeme.start) - echeme.start),2),
            n.trains = n(),
            train.rate = round(n() / ((max(train.end)/1000) - (min(train.start)/1000)),2),
            duty.cycle = round((sum(train.duration) / echeme.duration)*100,2)
          ) %>%
          mutate(
            echeme.period = lead(echeme.start) - echeme.start
          ) %>%
          ungroup()
      ...
      observeEvent(input$run, {
        ...
        output$echeme_data <- renderTable({
          result$echeme_data
        })
        ...
        output$downloadData <- downloadHandler(
          filename = function() {
            paste("temporal_stats_", Sys.Date(), ".xlsx", sep = "")
          },
          content = function(file) {
            wb <- createWorkbook()
            addWorksheet(wb, "echeme_data")
            addWorksheet(wb, "train_data")
            addWorksheet(wb, "peak_data")
            addWorksheet(wb, "params")
            writeData(wb, "echeme_data", result$echeme_data)
            writeData(wb, "train_data", result$train_data)
            writeData(wb, "peak_data", result$peak_data)
            writeData(wb, "params", result$params)
    ...
    brochure::page(
          ...
          fluidPage(
            ...
            fluidRow(
              column(
                ...
                selectInput("selectedTable", "Select an analysis table:", choices = NULL, width = '100%'),
                actionButton("refresh", "Refresh List"),
                actionButton("spectro", "Plot Spectrogram"),
                checkboxInput("meanspec", "Add Mean Spectrum")
              ),
              column(3, verticalLayout(
                actionButton("clearPlot", "Clear Plot")
              )),
              ...
              column(
                2,
                verticalLayout(
                  numericInput("imgWidth", "Image Width (in):", value = 10, min = 1),
                  numericInput("imgHeight", "Image Height (in):", value = 2.5, min = 1),
            ...
            mainPanel(
              plotOutput("specPlot", height = "auto", width = "auto"),
              div(id = "spinner")  # Add the spinner div
            )
          )
        )
      },
      server = function(input, output, session) {
        ...
        spectrogram <- function(wave, floor = -50, meanspec = TRUE) {
          # Set window length and hop length
          win_len <- 0.005 * wave@samp.rate
          hop_len <- 0.002 * wave@samp.rate
          overlap <- ((win_len - hop_len) / win_len) * 150
          ...
          # Apply floor to amplitude values
          dyn = -50
          spect_df_floor <- spect_df |> 
            mutate(
              amp_floor = case_when(
          ...
          if(meanspec){
            # Calculate mean spectrum using seewave's meanspec function
          ...
          spect_plot <- spect_df_floor |> 
            ...
            scale_fill_gradient(low = "white", high = "black", limits = c(dyn, max(spect_df$amp)), na.value = "white") +
        ...
        # List all Wave objects in the environment for the select input
        observe({
          waveObjects <- ls(envir = .GlobalEnv)[sapply(ls(envir = .GlobalEnv), function(x)
            inherits(get(x, envir = .GlobalEnv), "Wave"))]
          updateSelectInput(session, "waveObject", choices = waveObjects)
          
          analysisTables <- ls(envir = .GlobalEnv)[sapply(ls(envir = .GlobalEnv), function(x)
            is.data.frame(get(x, envir = .GlobalEnv)))]
          updateSelectInput(session, "selectedTable", choices = analysisTables)
        })
        ...
        # Update plot parameters and cache the spectrogram when "Plot Spectrogram" is clicked
        observeEvent(input$spectro, {
          ...
          # Show the spinner
          session$sendCustomMessage(type = 'toggleSpinner', message = TRUE)
          
          # Generate and cache the spectrogram
          output$specPlot <- renderPlot({
            tryCatch({
              plotParams <- spectrogramCache()
              combined_plot <- spectrogram(plotParams$wave, meanspec = plotParams$meanspec)
              session$sendCustomMessage(type = 'toggleSpinner', message = FALSE)
              print(combined_plot)
            }, error = function(e) {
              session$sendCustomMessage(type = 'toggleSpinner', message = FALSE)
              showModal(modalDialog(
    ...
    brochure::page(
      ...
      ui = function(request) {
        tagList(
          ...
          div(style = "padding-left: 15px;padding-right: 15px;",
              fluidRow(
                column(
                  ...
                  selectInput("waveObject", "Select a wave object:", 
                              choices = NULL, width = '100%'),
                  selectInput("selectedTable", "Select an analysis table:", 
                              choices = NULL, width = '100%')
                ),
                column(3, verticalLayout(
                  actionButton("refresh", "Refresh List"),
                  actionButton("oscillogram", "Plot Oscillogram"),
                  actionButton("zoomedPortion", "Zoomed Portion")
                  # useShinyjs(),
          ...
          div(style = "padding-left: 15px; padding-right: 15px; margin-top = 5px",
              mainPanel(
                div(style = "margin-bottom: 5px;", # Add space between plots
      ...
      server = function(input, output, session) {
        ...
        # observeEvent(input$clearPlot, {
        #   session$sendCustomMessage(type = 'refreshPage', message = TRUE)
        # })
    ...
    brochure::page(
      ...
      server = function(input, output, session) {
        ...
        spectroscillo <- function(wave, floor = -50) {
          ...
          spect_plot <- spect_df_floor |> ggplot(aes(time, freq)) +
            geom_raster(aes(fill = amp_floor)) +
            scale_fill_gradient(low = "white", high = "black", limits = c(dyn, max(spect_df$amp)), na.value = "white") +
            coord_cartesian(expand = FALSE) +
            theme_minimal(base_size = 15) +
            ...
            labs(x = "Time (s)", y = "Frequency (kHz)", title = "")
          
          spectrum_plot <- mean_spectrum_df |> ggplot(aes(x = mean_amp, y = freq)) +
            geom_line(color = "black") +
            geom_ribbon(aes(ymin = 0, ymax = freq), fill = "black") +
            theme_minimal(base_size = 15) +
            scale_y_continuous(n.breaks = 3, expand = c(0, 0)) +
            ...
            labs(x = NULL, y = "Mean Amplitude", title = "") +
            coord_flip()
        ...
        observe({
          ...
          analysisTables <- ls(envir = .GlobalEnv)[sapply(ls(envir = .GlobalEnv), function(x) is.data.frame(get(x, envir = .GlobalEnv)))]
          updateSelectInput(session, "selectedTable", choices = analysisTables)
        })
  ...
  page_11 <- function() {
    brochure::page(
        ...
        tagList(
          ...
          fluidPage(
            ...
            fluidRow(
              column(
                4,
                selectInput("waveObjects", "Select wave objects:", choices = NULL, selected = NULL, multiple = TRUE, width = '100%'),
                numericInput("maxLength", "Max Length (s):", value = 1, min = 0.5, step = 0.5, width = '100%'),
                ...
                actionButton("refresh", "Refresh List"),
                actionButton("plotMultiOscillogram", "Plot Multi-Oscillogram"),
                checkboxInput("transparentBg", "Transparent Background", value = FALSE),
                downloadButton("saveImage", "Save Image")
              ),
              column(
      ...
      server = function(input, output, session) {
        ...
        output$plotOutput <- renderUI({
          req(result())
          plotOutput("multiWavePlot", height = paste0(100 * length(input$waveObjects), "px"), width = "1450px")
        })
